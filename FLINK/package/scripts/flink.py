...
class Master(Script):
  def install(self, env):
    ...
    if params.setup_prebuilt:
      ...
      Execute('cd '+params.flink_install_dir+'; mvn clean install -DskipTests -Dhadoop.version=3.1.1 -Pvendor-repos >> ' + params.flink_log_file, user=params.flink_user)
  ...
  def start(self, env):
    ...
    cmd_open = subprocess.Popen(["/bin/sh", "/var/lib/ambari-server/resources/stacks/CRH/8.3.1.1/services/FLINK/package/scripts/get_evn.sh"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
